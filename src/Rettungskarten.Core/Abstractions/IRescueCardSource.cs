...
/// step that can fail per-model (e.g. VW's 403) and reports failure via the result, not an exception.
/// A brand with no working source yet (e.g. Porsche) throws <see cref="NotSupportedException"/> from
/// <see cref="DiscoverAsync"/> so the orchestrator can record it as "not implemented" rather than a failure.
/// </summary>
